    ...
    class NestedClass(
            ...
            while (usageIterator.hasNext()) {
                ...
                when {
                    fullCall is KtQualifiedExpression -> {
                        conflicts.putValue(
                                fullCall,
                                "Qualified call won't be processed by the Move refactoring: ${fullCall.text}"
                        )
                        usageIterator.remove()
                    }
                    ...
                    usage.isDoubleReceiver -> {
                        conflicts.putValue(
                                fullCall,
                                "Call with two implicit receivers won't be processed by the Move refactoring: ${fullCall.text}"
                        )
                        usageIterator.remove()
                    }
                }
        ...
        override fun preprocessDeclaration(descriptor: MoveDeclarationsDescriptor, originalDeclaration: KtNamedDeclaration) {
            with(originalDeclaration) {
                ...
                if (this is KtClass) {
                    if (isInner() && (descriptor.moveTarget as? KotlinMoveTargetForExistingElement)?.targetElement !is KtClass) {
                        removeModifier(KtTokens.INNER_KEYWORD)
                    }
                    ...
                    if (outerInstanceParameterName != null) {
                        ...
                        val parameter = KtPsiFactory(project)
                                .createParameter("val $outerInstanceParameterName: ${IdeDescriptorRenderers.SOURCE_CODE.renderType(type)}")
