...
public class JavaPsiFacadeKotlinHacks extends JavaPsiFacadeEx {
    ...
    public JavaPsiFacadeKotlinHacks(Project project) {
        myProject = project;
        this.myFileManager = findJavaFileManager(project);
        this.isCoreJavaFileManager = myFileManager instanceof CoreJavaFileManager;
        ...
        MessageBus bus;
        try {
            Field busField = psiManager.getClass().getDeclaredField("myMessageBus");
            busField.setAccessible(true);
            bus = (MessageBus) busField.get(psiManager);
        }
        catch (NoSuchFieldException e) {
            throw new IllegalStateException("Can't find field: " + e);
        }
        catch (IllegalAccessException e) {
            throw new IllegalStateException("Can't get access to field: " + e);
        }
    ...
    private PsiElementFinder[] calcFinders() {
        ...
        List<PsiElementFinder> filteredExtension =
                ContainerUtil.filter(myProject.getExtensions(PsiElementFinder.EP_NAME), new Condition<PsiElementFinder>() {
                    @Override
                    public boolean value(PsiElementFinder finder) {
                        return !(finder instanceof KotlinFinderMarker);
                    }
                });

        ContainerUtil.addAll(elementFinders, filteredExtension);
    ...
    private class PsiElementFinderImpl extends PsiElementFinder implements DumbAware {
        ...
        public PsiClass findClass(@NotNull String qualifiedName, @NotNull GlobalSearchScope scope) {
            PsiClass aClass = myFileManager.findClass(qualifiedName, scope);
            if (aClass != null) {
        ...
        public PsiClass[] findClasses(@NotNull String qualifiedName, @NotNull final GlobalSearchScope scope) {
            ...
            return ArrayUtil.toObjectArray(
                    KotlinPackage.filter(myFileManager.findClasses(qualifiedName, scope), new Function1<PsiClass, Boolean>() {
                        @Override
                        public Boolean invoke(PsiClass aClass) {
                            return scope.contains(aClass.getContainingFile().getOriginalFile().getVirtualFile());
                        }
                    }), PsiClass.class);
        }
