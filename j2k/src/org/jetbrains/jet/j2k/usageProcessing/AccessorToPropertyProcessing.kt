...
class AccessorToPropertyProcessing(val accessorMethod: PsiMethod, val accessorKind: AccessorKind, val propertyName: String) : UsageProcessing {
    ...
    override val convertedCodeProcessor: ConvertedCodeProcessor? = object: ConvertedCodeProcessor {
        override fun convertMethodUsage(methodCall: PsiMethodCallExpression, converter: Converter): Expression? {
            val isNullable = converter.typeConverter.methodNullability(accessorMethod).isNullable(converter.settings)
            ...
            val propertyAccess = QualifiedExpression(converter.convertExpression(methodExpr.getQualifierExpression()), propertyName).assignNoPrototype()
            ...
            if (accessorKind == AccessorKind.GETTER) {
                ...
                val argument = converter.convertExpression(arguments[0])
    ...
    override val kotlinCodeProcessor: ExternalCodeProcessor? = object: ExternalCodeProcessor {
        override fun processUsage(reference: PsiReference) {
            val v = reference.getElement() as JetSimpleNameExpression
        }
    }
}