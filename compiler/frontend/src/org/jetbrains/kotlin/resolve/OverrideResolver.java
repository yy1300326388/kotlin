...
public class OverrideResolver {
    ...
    private static void checkInheritedSignaturesForFakeOverride(
        ...
        assert memberDescriptor.getKind() == FAKE_OVERRIDE
                : "Fake override expected; actual: " + memberDescriptor + " of kind " + memberDescriptor.getKind();

        int numInheritedMembers = relevantDirectlyOverridden.size();
        if (numInheritedMembers > 1) {
            List<CallableMemberDescriptor> inheritedMembers = new ArrayList<CallableMemberDescriptor>(relevantDirectlyOverridden);
            boolean isProperty = memberDescriptor instanceof PropertyDescriptor;

            for (int i1 = 0; i1 < numInheritedMembers - 1; ++i1) {
                CallableMemberDescriptor descriptor1 = inheritedMembers.get(i1);
                for (int i2 = i1 + 1; i2 < numInheritedMembers; ++i2) {
                    CallableMemberDescriptor descriptor2 = inheritedMembers.get(i2);
                    if (isProperty) {
                        assert descriptor1 instanceof PropertyDescriptor
                                : "Member " + descriptor1 + "overridden by " + memberDescriptor + " is not a property";
                        assert descriptor2 instanceof PropertyDescriptor
                                : "Member " + descriptor2 + "overridden by " + memberDescriptor + " is not a property";

                        if (!isPropertyTypeOkOnInheritance((PropertyDescriptor) descriptor1, (PropertyDescriptor) descriptor2)) {
                            reportingStrategy.clashingWithPropertyType(descriptor1, descriptor2);
                            break;
                        }
                    }
                    else if (!isReturnTypeOkOnInheritance(descriptor1, descriptor2)) {
                        reportingStrategy.clashingWithReturnType(descriptor1, descriptor2);
                        break;
                    }
                }
            }
    ...
    private static boolean isPropertyTypeOkOnInheritance(PropertyDescriptor descriptor1, PropertyDescriptor descriptor2) {
        KotlinType returnType1 = descriptor1.getReturnType();
        assert returnType1 != null : "Return type for " + descriptor1 + " is null";

        KotlinType returnType2 = descriptor2.getReturnType();
        assert returnType2 != null : "Return type for " + descriptor2 + " is null";

        if (descriptor1.isVar() || descriptor2.isVar()) {
            return KotlinTypeChecker.DEFAULT.equalTypes(returnType1, returnType2);
        }
        else {
            return KotlinTypeChecker.DEFAULT.isSubtypeOf(returnType1, returnType2) ||
                   KotlinTypeChecker.DEFAULT.isSubtypeOf(returnType2, returnType1);
        }
    }

    private static boolean isReturnTypeOkOnInheritance(CallableMemberDescriptor descriptor1, CallableMemberDescriptor descriptor2) {
        KotlinType returnType1 = descriptor1.getReturnType();
        assert returnType1 != null : "Return type for " + descriptor1 + " is null";
