...
public class QualifiedExpressionResolver(val symbolUsageValidator: SymbolUsageValidator) {
    ...
    public fun resolveDescriptorForUserType(
        ...
        if (hasError) {
            resolveToPackageOrClass(qualifierPartList, module, trace, scope.ownerDescriptor, scope, position = QualifierPosition.TYPE)
            return null
        }
    ...
    private fun resolveToPackageOrClassPrefix(
        ...
        if (position == QualifierPosition.EXPRESSION) {
            ...
            // If we see any kind of value (possibly ambiguous),
            // tell resolver we have no qualifier and let it perform the context-dependent resolution.
            val valueDescriptor = scopeForFirstPart?.let {
                it.findVariable(firstPart.name, firstPart.location) ?:
                it.findFunction(firstPart.name, firstPart.location)
            }
