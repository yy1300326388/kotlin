...
public class QualifiedExpressionResolver(val symbolUsageValidator: SymbolUsageValidator) {
    ...
    private fun storageQualifier(trace: BindingTrace, referenceExpression: KtSimpleNameExpression, descriptor: DeclarationDescriptor) {
        val qualifier =
                when (descriptor) {
                    ...
                    is ClassifierDescriptor -> createClassifierQualifier(referenceExpression, descriptor)
                    else -> return
                }
    ...
    public fun resolveAsReceiverInQualifiedExpression(
        ...
        if (qualifier is ClassQualifier) {
            if (qualifier.classifier is TypeParameterDescriptor) {
                context.trace.report(Errors.TYPE_PARAMETER_ON_LHS_OF_DOT.on(qualifier.referenceExpression, qualifier.classifier))
            }
            else if (qualifier.classifier is ClassDescriptor && qualifier.classifier.hasClassObjectType) {
                context.trace.recordType(qualifier.expression, qualifier.classifier.classObjectType)
            }
        }
    }
    ...
    public fun resolveAsStandaloneExpression(qualifier: QualifierReceiver, context: ExpressionTypingContext) {
        ...
        if (qualifier is ClassQualifier) {
            if (qualifier.classifier is TypeParameterDescriptor) {
                context.trace.report(Errors.TYPE_PARAMETER_IS_NOT_AN_EXPRESSION.on(qualifier.referenceExpression, qualifier.classifier))
            }
            else if (qualifier.classifier is ClassDescriptor && !qualifier.classifier.hasClassObjectType) {
                context.trace.report(Errors.NO_COMPANION_OBJECT.on(qualifier.referenceExpression, qualifier.classifier))
            }
        }
