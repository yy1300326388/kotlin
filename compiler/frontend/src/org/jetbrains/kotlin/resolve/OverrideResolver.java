...
public class OverrideResolver {
    ...
    private static void checkInheritedSignaturesForFakeOverride(
        ...
        assert memberDescriptor.getKind() == FAKE_OVERRIDE : "Fake override expected; actual: " + memberDescriptor +
                                                             " of kind " + memberDescriptor.getKind();

        assert !relevantDirectlyOverridden.isEmpty() : "Fake override should override something; descriptor: " + memberDescriptor;

        boolean isProperty = memberDescriptor instanceof PropertyDescriptor;

        if (relevantDirectlyOverridden.size() > 1) {
            for (CallableMemberDescriptor descriptor1 : relevantDirectlyOverridden) {
                for (CallableMemberDescriptor descriptor2 : relevantDirectlyOverridden) {
                    if (descriptor1 == descriptor2) continue;
                    if (isProperty) {
                        assert descriptor1 instanceof PropertyDescriptor : "Overridden member " + descriptor1 + " is not a property";
                        assert descriptor2 instanceof PropertyDescriptor : "Overridden member " + descriptor2 + " is not a property";
