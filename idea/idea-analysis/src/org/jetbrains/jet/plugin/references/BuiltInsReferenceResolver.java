    ...
    private void initialize() {
        ...
        final Runnable initializeRunnable = new Runnable() {
            ...
            public void run() {
                ...
                // TODO: check if it can cause problems with plugin start
                assert fragments.size() == 1 : "Only one package fragment is expected for builtins";

                builtinsPackageFragment = fragments.size() == 1 ?
                                          KotlinPackage.first(fragments) :
                                          new MutablePackageFragmentDescriptor(module, KotlinBuiltIns.BUILT_INS_PACKAGE_FQ_NAME);
