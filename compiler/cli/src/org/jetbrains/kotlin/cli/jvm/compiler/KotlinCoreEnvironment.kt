    ...
    private fun fillClasspath(configuration: CompilerConfiguration) {
        for (root in configuration.getList(CommonConfigurationKeys.CONTENT_ROOTS)) {
            ...
            javaRoots.add(
                    when (javaRoot) {
                        is JavaSourceRoot -> {
                            val packagePrefixFqName = javaRoot.packagePrefix?.let {
                            ...
                            JavaRoot(virtualFile, JavaRoot.RootType.SOURCE, packagePrefixFqName)
                        }
                        is JvmClasspathRoot -> JavaRoot(virtualFile, JavaRoot.RootType.BINARY)
