...
class OverloadingConflictResolver(private val builtIns: KotlinBuiltIns) {
    ...
    private fun <D : CallableDescriptor> findMaximallySpecificCall(
        ...
        return conflictingCandidates.mapNotNull {
            candidate ->
            candidate.check {
                isMaxSpecific(candidate, conflictingCandidates) {
                    call1, call2 ->
                    isNotLessSpecificConflictingCall(call1, call2, discriminateGenericDescriptors)
                }
            }?.resolvedCall
        }.singleOrNull()
    }
     ...
     */
    private fun <D : CallableDescriptor, K> isNotLessSpecificConflictingCall(
    ...
    /**
     * Returns `true` if `d1` is definitely more specific then `d2`,
     * `false` if `d1` is definitely less specific then `d2`,
     * `null` if undecided.
     */
    private fun <D : CallableDescriptor, K> compareConflictingCalls(
        ...
        if (!hasVarargs1 && !hasVarargs2 && call1.unusedParametersCount > call2.unusedParametersCount) return false
    ...
    /**
     * Returns `false` if `d1` is definitely less specific then `d2`,
     * `null` if undecided.
     */
